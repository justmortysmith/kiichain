...
func (s *IntegrationTestSuite) testLSM() {
	...
	// Validator bond
	s.executeValidatorBond(s.chainA, 0, validatorAddressA, validatorAAddr.String(), kiichainHomePath, fees.String())
	...
	tokenize := sdk.NewCoin(akiiDenom, tokenizeAmount) // 200 atom
	s.executeTokenizeShares(s.chainA, 0, tokenize.String(), validatorAddressA, delegatorAddress.String(), kiichainHomePath, fees.String())
	...
	s.executeTransferTokenizeShareRecord(s.chainA, 0, strconv.Itoa(recordID), delegatorAddress.String(), validatorAAddr.String(), kiichainHomePath, standardFees.String())
	...
	s.executeRedeemShares(s.chainA, 0, redeemAmount.String(), validatorAAddr.String(), kiichainHomePath, fees.String())
	...
	s.Require().Eventually(
		func() bool {
			...
			balanceRes, err = getSpecificBalance(chainEndpoint, tokenizeShareRecord.GetModuleAddress().String(), akiiDenom)
			s.Require().NoError(err)
			if balanceRes.Amount.IsNil() || balanceRes.Amount.IsZero() {
				return false
			}
